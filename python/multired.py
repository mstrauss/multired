...
#  -- 2018/10/05 -- upgrade to Python 3
...
class XLogx_fit:
    ...
    def __getitem__(self, index):
        if index <= self.degree:
            ...
            print("Error!!! Index %d is larger than the degree of the fitting "
                  "polynomial (%d)"
                  % (index, self.degree))
            sys.exit(-1)
...
class layer:
    ...
    def __init__(self, layerfile=None, matrix=None):
        ...
        if layerfile is not None:
            ...
            D = csr_matrix(
                (K, (range(self.N), range(self.N))), shape=(self.N, self.N))
...
class multiplex_red:
    ...
    def __init__(self, multiplexfile,
                 directed=None,
                 fit_degree=10,
                 verbose=False):
        self.layers = []
        ...
        try:
            ...
            print("Unable to find/open file %s -- Exiting!!!" % multiplexfile)
            exit(-2)
    ...
    def compute_multiplex_entropy(self, force_compute=False):
        # The entropy of a multiplex is defined as the sum of the entropies of
        # its layers
        for l in self.layers:
    ...
    def dump_JSD(self, force_compute=False):
        if self.JSD is None:
            if force_compute:
                ...
                print("Error!!! call to dump_JSD but JSD matrix has not been "
                      "computed!!!")
                sys.exit(1)
        idx = 0
    ...
    def dump_JSD_approx(self, force_compute=False):
        if self.JSD_approx is None:
            if force_compute:
                ...
                print("Error!!! call to dump_JSD_approx but JSD approximate "
    ...
    def draw_dendrogram_approx(self, force=False,
        ...
        if self.Z_approx is None:
            if not force:
                sys.stderr.write(
                    "Please call reduce_approx() first or specify "
                    "'force=True'")
            else:
                self.reduce_approx()
